...
public class Server {
    ...
    public static final int COMMAND_STATE = 555; // 在线状态更新命令
    public static final int LOGIN_SUCCESS = 001; // 登陆成功
    public static final int LOGIN_FAILURE = 000; // 登陆失败
    ...
    // ============================供解析器parser调用
    // 1. 获取带有在线状态的好友列表
    public static List<Map<String, String>> getFriendsListWithState(String userId) {
            ...
            try {
                ...
                if (cmd == Server.COMMAND_LOGIN) {
                    ...
                    if (user != null && jsonObj.get("user_pwd").equals(user.get("user_pwd"))) {
                        // Tip: 这里利用了if的短路判断，必须首先判断用户存在与否，存在再判断密码正确与否
                        ...
                        sendJsonObj.put("result", Server.LOGIN_SUCCESS);
                        List<Map<String, String>> friends = Server.getFriendsListWithState(userId);
                        ...
                        // 向其他用户广播此用户上线信息
                        Server.broadcastStateChange(userId, Server.ONLINE);
                        ...
                        sendJsonObj.put("result", Server.LOGIN_FAILURE);
                        byte[] b = sendJsonObj.toString().getBytes();
                        packet = new DatagramPacket(b, b.length, address, port);
                        Server.socket.send(packet);
                    }

                } else if (cmd == Server.COMMAND_SENDMSG) {
